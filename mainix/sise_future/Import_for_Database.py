...
class stock_future():
    ...
    def shinhan_wti(self):
        ...
        shinhan_wti_etn="500015"
        shinhan_wti_etn_2x = "500019"
        shinhan_wti_etn_inverse = "500003"
        shinhan_wti_etn_inverse_2x = "500027"
        list = []
        code_list = [{"stock_name":"신한 WTI ETN","code":shinhan_wti_etn}
                        ,{"stock_name":"신한 WTI 2X ETN","code":shinhan_wti_etn_2x}
                        ,{"stock_name":"신한 WTI 인버스 ETN","code":shinhan_wti_etn_inverse}
                        ,{"stock_name":"신한 WTI 2X ETN","code":shinhan_wti_etn_inverse_2x}]
        if weekday > 4:
            for data in code_list:
                result = self.stock_naver_sise(data["code"])
                result[0]["stock_name"] = data["stock_name"]
                data = {'stock_name': result[0]['stock_name'], 'sise': result[0]["sise"],
                        'dungrak': 0 , 'code': data["code"], 'time': "장마감"}
                list.append(data)
        elif int(time[:2]) > 8 and int(time[:2]) < 17:
            for data in code_list:
                result = self.stock_naver_sise(data["code"])
                result[0]["stock_name"] = data["stock_name"]
                data = {'stock_name': result[0]['stock_name'], 'sise': result[0]["sise"],
                       'dungrak': float(result[0]["dungrak"]), 'code': data["code"], 'time': time}
                list.append(data)
...
if __name__=="__main__":
    ...
    print(stock_future.shinhan_wti())
